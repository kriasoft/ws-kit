...
describe("Delta Sync Conformance", () => {
  ...
  it("fixtures have matching schema versions", async () => {
    ...
    const fixtures = await Promise.all([
      import("./fixtures/001-happy-path.json").then((m) => m.default),
      import("./fixtures/002-revision-gap.json").then((m) => m.default),
      import("./fixtures/003-optimistic-update.json").then((m) => m.default),
    ]);
  ...
  it("fixtures are numbered sequentially", async () => {
    const fixtures = [
      "./fixtures/001-happy-path.json",
      "./fixtures/002-revision-gap.json",
      "./fixtures/003-optimistic-update.json",
    ];
