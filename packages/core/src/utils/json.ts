 ...
 */
export function safeJsonParse(data: string, maxBytes?: number): ParseOutcome {
  // Check size first
...
export function safeJsonStringify(obj: unknown, maxBytes?: number): string {
  const json = JSON.stringify(obj);
  if (maxBytes && json.length > maxBytes) {
    throw new Error(
      `Serialized message exceeds max payload size: ${json.length} > ${maxBytes}`,
    );
  }
  return json;
}
