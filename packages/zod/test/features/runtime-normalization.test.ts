...
/**
 * Runtime Normalization Integration Tests (Zod)
 *
 * Validates that the Zod validator correctly receives normalized messages
...
describe("Runtime Normalization (Zod Validator)", () => {
  describe("Validator Receives Normalized Messages", () => {
    it("should validate message after clientId is stripped from meta", () => {
      ...
      // Zod validator should receive normalized message and accept it
  ...
  describe("Contract Validation", () => {
    it("should maintain validator contract: normalized input always passes validation (if schema matches)", () => {
      const TestMsg = message("TEST", { count: z.number().positive() });
    ...
    it("should maintain validator contract: invalid payload still fails after normalization", () => {
      ...
      const malicious = {
        type: "TEST",
        meta: { clientId: "fake" },
        payload: { count: -5 }, // Invalid: must be positive
      };
