...
describe("ctx.progress() - throttling support", () => {
  describe("throttleMs option basic behavior", () => {
    it("throttleMs parameter accepted and processed", async () => {
      const Request = rpc("REQUEST", { count: z.number() }, "PROGRESS", {
        current: z.number(),
      });
  ...
  describe("real-world patterns", () => {
    it("animation frame updates with throttling", async () => {
      const Request = rpc("REQUEST", { frameCount: z.number() }, "PROGRESS", {
        frame: z.number(),
        position: z.number(),
      });
      const router = createRouter().plugin(withZod());
    ...
    it("sensor data updates with throttling", async () => {
      const Request = rpc("REQUEST", { duration: z.number() }, "PROGRESS", {
        reading: z.number(),
        timestamp: z.number(),
      });
      const router = createRouter().plugin(withZod());
    ...
    it("bulk processing with progress metrics", async () => {
      const Request = rpc("REQUEST", { itemCount: z.number() }, "PROGRESS", {
        processed: z.number(),
        total: z.number(),
        percentage: z.number(),
      });
