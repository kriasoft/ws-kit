...
import { message, rpc, withValibot } from "@ws-kit/valibot";
...
describe("reply() and progress() runtime", () => {
  ...
  describe("plugin integration", () => {
    it("should add rpc method after withValibot()", () => {
      const router = createRouter().plugin(withValibot());
  ...
  describe("outgoing validation", () => {
    it("should validate outgoing by default", async () => {
      ...
      const router = createRouter().plugin(
        withValibot({ validateOutgoing: true }),
      );
    ...
    it("should skip validation when disabled", async () => {
      ...
      const router = createRouter().plugin(
        withValibot({ validateOutgoing: false }),
      );
  ...
  describe("send() method", () => {
    it("should validate outgoing messages when enabled", async () => {
      const Join = message("JOIN", { roomId: v.string() });
      const Joined = message("JOINED", { userId: v.string() });
